...
# um subconjunto.
def gen_subsets(n, subset_min_size):
    n_set = list(range(1,n+1)) # Universo
    subsets = [] # Lista de subconjuntos
    
    # Cria 'n' subconjuntos
    for _ in range(n):
        # Cada subconjunto terá um tamanho aleatório maior ou igual ao tamanho mínimo
        subset_size = random.randint(math.ceil(subset_min_size*n), n) 
...
def create_instance(filename_suffix):
    ...
    base_dir = os.path.join(".", "data", "instances", "instances2")
    ...
    subsets = gen_subsets(n, subset_min_size)
    ...
    with open(filename, "a") as file:
        ...
        for i in range(n, 0, -1):
            for j in range(i):
                file.write(str(round(random.uniform(-10.0, 10.0), 2)) + " ")
...
# Obtém o número de instâncias a serem criadas a partir do primeiro argumento da linha de comando
# Obtém o "tamanho mínimo" de cada subconjunto a partir do segundo argumento da linha de comando. 
# Ex: 'python instance_gen2.py 5 0.8' criará 5 instâncias nas quais os subconjuntos tem tamanho >= 0.8n.
num_instances = int(sys.argv[1])
subset_min_size = float(sys.argv[2]) #se for 0.8 significa que cada subconjunto tem tamanho pelo menos 0.8n 

# Laço para chamar a função 'create_instance' o número de vezes especificado.
for instance in range(1, num_instances+1):
    create_instance(instance) # estou passando o número da instância como argumento só pra nomear o arquivo
